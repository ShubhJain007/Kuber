...
\textit{Future work.} Several directions follow directly from the analysis above; we state each with its
rationale so results can be filled in as runs complete. \textbf{(i)~Anisotropic Boundary-Layer Attention
(ABL):} replace the isotropic $k$-NN in the surface cross-attention with a wall-normal-penalized score,
$\alpha_{ji}\!\leftarrow\!\mathrm{softmax}(\dots-\gamma\,|(\mathbf{x}_j-\mathbf{p}_i)\cdot\mathbf{n}_i|)$,
so boundary-layer queries attend \emph{along} the wall rather than across it---targeting the near-wall
temperature error that dominates our residual. \textbf{(ii)~Buckingham-$\Pi$ conditioning:} condition on the
dimensionless groups (Re,~Ra,~Pe; Pr is already included) rather than raw fluid properties, so a new fluid
is a point in similarity space, enabling cross-fluid transfer. \textbf{(iii)~Leave-One-Fluid-Out (LOFO):}
train on air and water and evaluate zero-shot on high-viscosity oil ($\mathrm{Pr}\approx292$), a stronger
out-of-distribution axis than fin count. \textbf{(iv)~Interface flux-balanced loss:} a soft residual
...
\textbf{(v)~Physics-masked pretraining:} a self-supervised objective reconstructing masked spatial regions
