...
Table~\ref{tab:abl} isolates the main design choices on the SIMSHIFT medium split. \textbf{Anisotropic
Boundary-Layer attention is KuberNet's largest single design win}: ablating it to isotropic kNN raises the
out-of-distribution temperature from 11.84 to 12.14\,K (mean nRMSE $0.608\!\to\!0.671$) at equal parameter
count. Adding the PDE-Refiner head is fine in-distribution (4.15\,K) but degrades OOD to 16.52\,K---its
iterative refinement amplifies error under distribution shift---which is why we report the one-shot model.
The surface encoder gives the best temperature and, unlike the analytic SDF, applies to arbitrary CAD; the SDF variant is
...
attention & KuberNet (ABL) & \textbf{11.84} \\
 & w/o ABL (isotropic kNN) & 12.14 \\
output head & one-shot (ours) & \textbf{11.84} \\
 & + PDE-Refiner & 16.52 \\
