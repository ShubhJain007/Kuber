...
\textit{Future work.} The most direct gains are scale---a larger surface-conditioned core (more physics
slices, depth, and hidden width) and a substantially larger, less air-dominated corpus, both of which our
value-of-data ablation suggests still pay off. On architecture, hierarchical multi-resolution surface
tokens and per-block geometry cross-attention should sharpen near-wall fields, and the PDE-Refiner head
extends naturally to calibrated per-node uncertainty. On coverage we are adding cold-plate topologies
(serpentine, pin-fin, parallel micro-channel), further device classes, and harder leave-one-fluid/shape-out
splits. Toward production, the surrogate exports to ONNX/TensorRT for batched multi-geometry inference and
drops into an agentic propose--predict--score--refine loop for geometry optimization, with CAD connectors
(STEP/STL/mesh) for direct ingest.
